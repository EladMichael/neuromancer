...
If you are adapting this framework to a new building dataset, read `docs/orchestrator.md` first. It contains the full phase sequence, file map, and run commands. Load one phase document at a time to keep your context lean.

If you are modifying framework internals (anything in `src/` other than `user_defined.py` and `configs/`), read the relevant source files first and follow the coding rules below.
